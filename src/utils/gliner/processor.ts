 ...
 * Original source: https://github.com/Knowledgator/GLiNER.js/blob/main/src/lib/processor.ts
*/
...
export class Processor {
    ...
    constructor(tokenizer: Tokenizer, wordsSplitter: WhitespaceTokenSplitter) {
...
export class SpanProcessor extends Processor {
    maxWidth: number;
    constructor(tokenizer: any, wordsSplitter: WhitespaceTokenSplitter, maxWidth: number) {
        super(tokenizer, wordsSplitter);
    ...
    prepareBatch(texts: string[], entities: string[]): ProcessBatch {
        ...
        let { spanIdxs, spanMasks }: { spanIdxs: number[][][]; spanMasks: boolean[][] } =
            this.prepareSpans(batchTokens, this.maxWidth);
